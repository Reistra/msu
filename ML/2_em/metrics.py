...
def compute_precision(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    for ref, pred in zip(reference, predicted):
        ...
        possible_set, sure_set = set(ref.possible), set(ref.sure)
        poss_sure_set = possible_set | sure_set
        intersection += len(predicted_set & poss_sure_set)
...
def compute_aer(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> float:
    ...
    """
    intersection_possible, total_predicted = compute_precision(reference, predicted)
    intersection_sure, total_sure = compute_recall(reference, predicted)

    numerator = (intersection_possible + intersection_sure) 
    denumerator = (total_predicted + total_sure)
    aer = 1 - (numerator/denumerator)
    return aer
